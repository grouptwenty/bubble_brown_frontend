...
class editView extends Component {
    ...
    async componentDidMount() {
        ...
        const instruction_data = await instruction_model.getInstructionByCode(code)
    //    console.log("code",code);
       
        this.setval(instruction_data.data)
    ...
    async setval(data) { //setdata edit
        document.getElementById('instruction_code').value = data.instruction_code
        document.getElementById('instruction_title_th').value = data.instruction_title_th
        document.getElementById('instruction_title_en').value = data.instruction_title_en
        document.getElementById('instruction_description_th').value = data.instruction_description_th
        document.getElementById('instruction_description_en').value = data.instruction_description_en
        
            if (data.instruction_img != null) {

                this.setState({
                    instruction_img_old: data.instruction_img
                })
    ...
    async handleSubmit(event) {
        ...
        if (this.state.selectedFile != null) {
            ...
            if (this.state.instruction_img_old != "" && this.state.instruction_img_old != null) {
                var req = await upload_model.deleteImages(this.state.instruction_img_old, "instruction")
            ...
            arr['instruction_img'] = this.state.instruction_img_old

        }

        console.log(this.check(arr))
        if (this.check(arr)) {
            var res = await instruction_model.updateInstructionBy(arr);
            //   console.log(res)
            if (res.data) {
                swal({
                    title: "Good job!",
                    text: "Update instruction Ok",
                    icon: "success",
                    button: "Close",
                });
...
let { imagePreviewUrl } = this.state;
        ...
        if (this.state.instruction_img_old != null && this.state.instruction_img_old != undefined && !imagePreviewUrl && this.state.instruction_img_old != '' ) {
            imagePreview = (<img className="responsive" style={{ width: '100%' }} src={GOBALS.URL_IMG + "instruction/" + this.state.instruction_img_old} />);
        }
