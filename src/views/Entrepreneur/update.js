...
class editView extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            user_img_old: "",
            refresh: false,
            entrepreneur_img_old: null,
            entrepreneur_password_old: ""
        };
    ...
    async componentDidMount() {
        ...
        const entrepreneur_data = await entrepreneur_model.getEntrepreneurByCode(code)
        ...
        this.setval(entrepreneur_data.data)
    ...
    async setval(data) { //setdata edit
        ...
        document.getElementById('entrepreneur_name_th').value = data.entrepreneur_name_th
        document.getElementById('entrepreneur_name_en').value = data.entrepreneur_name_en
        document.getElementById('entrepreneur_email').value = data.entrepreneur_email
        document.getElementById('entrepreneur_username').value = data.entrepreneur_username
        document.getElementById('entrepreneur_password').value = data.entrepreneur_password
        document.getElementById('entrepreneur_address').value = data.entrepreneur_address
        document.getElementById('entrepreneur_tel').value = data.entrepreneur_tel
        ...
        if (data.entrepreneur_img != null) {
            this.setState({
                entrepreneur_img_old: data.entrepreneur_img
            })
        }

        this.setState({
            entrepreneur_password_old: data.entrepreneur_password
        })
    }
    ...
    async handleSubmit(event) {
        ...
        if (this.check(arr)) {
            if (this.state.selectedFile != null) {
                ...
                if (this.state.entrepreneur_img_old != "" && this.state.entrepreneur_img_old != null) {
                    var req = await upload_model.deleteImages(this.state.entrepreneur_img_old, "entrepreneur")
                ...
                arr['entrepreneur_img'] = this.state.entrepreneur_img_old

            }

            if (arr['entrepreneur_password'] != this.state.entrepreneur_password_old) {
                arr['entrepreneur_password'] = await md5(arr['entrepreneur_password']);
            }
            var res = await entrepreneur_model.updateEntrepreneurBy(arr);
            //   console.log(res)
            if (res.data) {
                swal({
                    title: "Good job!",
                    text: "update entrepreneur Ok",
                    icon: "success",
                    button: "Close",
                });
    ...
    render() {
        ...
        if (this.state.entrepreneur_img_old != null && this.state.entrepreneur_img_old != undefined && !imagePreviewUrl && this.state.entrepreneur_img_old != '') {
            imagePreview = (<img className="responsive" style={{ width: '100%' }} src={GOBALS.URL_IMG + "entrepreneur/" + this.state.entrepreneur_img_old} />);
        }
                    ...
                    <Col>
                        <Card>
                            <Form onSubmit={this.handleSubmit} id="myForm">
                                <CardHeader>
                                    แก้ไขข้อมูลผู้ประกอบการ /  Edit Entrepreneur
